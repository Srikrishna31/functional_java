...
public class JavaRNGTest {
    ...
    public void testDoubleRnd() {
        RNG rng = JavaRNG.rng(0);
        Tuple<Double, RNG> result1 = Random.doubleRnd.run.apply(rng);
        assertTrue(Math.abs(-0.5380644351243973 - result1._1) < 0.0001);
        Tuple<Double, RNG> result2 = Random.doubleRnd.run.apply(result1._2);
        assertTrue(Math.abs(-0.3371180035173893 - result2._1) < 0.0001);
        Tuple<Double, RNG> result3 = Random.doubleRnd.run.apply(result2._2);
    ...
    public void testMap2() {
        ...
        Tuple<Integer, RNG> t = Random.integer.run.apply(result._2);
    ...
    public void testSequence() {
        RNG rng = JavaRNG.rng(0);
        Tuple<List<Integer>, RNG> result = Random.integersRnd.apply(3).run.apply(rng);
    ...
    public void testSequenceLength1() {
        RNG rng = JavaRNG.rng(0);
        Tuple<List<Integer>, RNG> result = Random.integersRnd.apply(1).run.apply(rng);
        assertEquals(List.list(-1155484576), result._1);
        Tuple<Integer, RNG> t = Random.integer.run.apply(result._2);
    ...
    public void testSequenceLength0() {
        RNG rng = JavaRNG.rng(0);
        Tuple<List<Integer>, RNG> result = Random.integersRnd.apply(0).run.apply(rng);
        assertEquals(0, result._1.length());
        Tuple<Integer, RNG> t = Random.integer.run.apply(result._2);
    ...
    public void testFlatMap() {
        ...
        Tuple<List<Integer>, RNG> result =
                Random.sequence(List.fill(300, () -> Random.notMultipleOfFiveRnd)).run.apply(rng);
